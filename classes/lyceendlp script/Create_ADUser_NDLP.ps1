...
# Obtention du chemin courant ou est éxécuté le script
...
$domaine = "NDLP"
$fqdn = "ndlp.fr"
$HomeDir = "\\serveurfic3\eleves" # Dossier de stockage des répertoires personnel
...
write-host "************************************************* supression de " $CSVPath
...
#		$nom = Remove-Diacritics([string]$nom)
		...
		$prenom = $prenom.Replace("é","e")	# ne fonctionne pas !
#		$prenom = Remove-Diacritics([string]$prenom)
		...
		Write-Host "Debug : " $nom $prenom $classe $login
